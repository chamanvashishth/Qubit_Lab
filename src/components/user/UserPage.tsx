        ...
        <div className="p-6 sm:p-10 border-b border-white/10">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div><p className="text-xs font-mono uppercase tracking-[.22em] text-zinc-500">{t('QubitLab account')}</p><h1 className="text-2xl sm:text-3xl font-semibold text-white mt-1">{user.name}</h1></div>
            </div>
            <button onClick={async () => { try { await logout(); onLoggedOut(); } catch (e) { setError(e instanceof Error ? e.message : 'Unable to log out.'); } }} className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-zinc-300 hover:text-white hover:bg-white/10 transition-colors"><LogOut className="w-4 h-4" /> {t('Sign out')}</button>
          </div>
        </div>
        <div className="p-6 sm:p-10 grid sm:grid-cols-2 gap-4">
          <div className="rounded-2xl border border-white/10 bg-black/25 p-5"><div className="flex items-center gap-2 text-zinc-400 text-xs font-mono uppercase tracking-wider"><Mail className="w-4 h-4" /> {t('Email')}</div><p className="mt-3 text-white break-all">{user.email}</p></div>
          <div className="rounded-2xl border border-white/10 bg-black/25 p-5"><div className="flex items-center gap-2 text-zinc-400 text-xs font-mono uppercase tracking-wider"><Check className="w-4 h-4" /> Account</div><p className="mt-3 text-white">{t('Active')}</p></div>
        </div>
  ...
  return <section className="max-w-5xl mx-auto py-8 sm:py-14">
    <div className="grid lg:grid-cols-[1fr_460px] gap-6 items-stretch">
      <div className="glass-section rounded-3xl p-7 sm:p-10 flex flex-col justify-between min-h-[520px]">
        <div><div className="w-12 h-12 rounded-2xl border border-[#dfff3f]/30 bg-[#dfff3f]/10 flex items-center justify-center mb-7"><Atom className="w-6 h-6 text-[#dfff3f]" /></div><p className="text-xs font-mono uppercase tracking-[.28em] text-[#dfff3f]">{t('Personal quantum workspace')}</p><h1 className="mt-3 text-4xl sm:text-5xl font-semibold tracking-tight text-white leading-tight">{t('Learn quantum computing with your progress in one place.')}</h1><p className="mt-5 max-w-xl text-zinc-400 leading-7">{t('Create an account to keep your QubitLab learning identity connected across sessions. Your tutor and interactive tools stay inside the same workspace.')}</p></div>
        <div className="mt-10 grid sm:grid-cols-2 gap-3 text-sm text-zinc-300">{[t('Personal learner profile'), t('Persistent Supabase session'), t('Progress-ready backend'), t('Managed authentication')].map((item) => <div key={item} className="flex items-center gap-2 rounded-xl border border-white/10 bg-black/20 px-4 py-3"><Check className="w-4 h-4 text-[#dfff3f]" />{item}</div>)}</div>
      </div>
      ...
      <div className="glass-section rounded-3xl p-6 sm:p-8 self-center">
        ...
        <div className="mb-6"><h2 className="text-2xl font-semibold text-white">{mode === 'login' ? t('Welcome back') : t('Create your account')}</h2><p className="mt-2 text-sm text-zinc-500">{mode === 'login' ? t('Continue your quantum learning workspace.') : t('Start with a free QubitLab learner account.')}</p></div>
        ...
        <p className="mt-5 text-center text-[11px] text-zinc-600">{t('Authentication is handled by Supabase Auth; passwords are not stored by QubitLab.')}</p>
      </div>
    </div>
  </section>;
};
