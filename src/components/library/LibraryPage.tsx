  ...
  <div className="space-y-10 py-7">
    ...
    <section>
      <div className="flex items-end justify-between gap-5">
        <div><h2 className="text-2xl font-semibold text-zinc-100">Study spaces</h2><p className="mt-2 text-sm text-zinc-400">Use QubitLab tools to learn, experiment and practice.</p></div>
    ...
    <section className="rounded-2xl border border-white/10 bg-white/[.035] p-6 sm:p-8">
      <div className="flex items-center gap-3"><GraduationCap className="w-5 h-5 text-violet-300" /><div><h2 className="text-xl font-semibold text-zinc-100">University Lecture Series</h2><p className="mt-1 text-sm text-zinc-500">Use these alongside the QubitLab curriculum for deeper study.</p></div></div>
      <div className="mt-6 grid gap-3 md:grid-cols-2">
        {lectureSeries.map(([title, description, href]) => <article key={title} className="rounded-xl border border-white/10 bg-black/20 p-5"><h3 className="text-sm font-semibold text-zinc-100">{title}</h3><p className="mt-2 text-xs leading-5 text-zinc-400">{description}</p><div className="mt-4"><ResourceLink href={href}>Open lectures</ResourceLink></div></article>)}
      </div>
    </section>

    <section className="rounded-2xl border border-white/10 bg-white/[.035] p-6 sm:p-8">
      <div className="flex items-center gap-3"><FileText className="w-5 h-5 text-emerald-300" /><div><h2 className="text-xl font-semibold text-zinc-100">Research & Reference</h2><p className="mt-1 text-sm text-zinc-500">Research and reference material explicitly linked in the supplied guide.</p></div></div>
      <div className="mt-6 grid gap-3 md:grid-cols-2">
        {researchAndMedia.filter(([title]) => !title.includes('Quantum Frontiers') && !title.includes('Aaronson')).map(([title, description, href]) => <article key={title} className="rounded-xl border border-white/10 bg-black/20 p-5"><h3 className="text-sm font-semibold text-zinc-100">{title}</h3><p className="mt-2 text-xs leading-5 text-zinc-400">{description}</p><div className="mt-4"><ResourceLink href={href}>Open reference</ResourceLink></div></article>)}
      </div>
    </section>

    <section className="rounded-2xl border border-white/10 bg-white/[.035] p-6 sm:p-8">
      <div className="flex items-center gap-3"><Newspaper className="w-5 h-5 text-orange-300" /><div><h2 className="text-xl font-semibold text-zinc-100">Articles, Blogs & Talks</h2><p className="mt-1 text-sm text-zinc-500">Accessible material for staying connected with the quantum-computing community.</p></div></div>
      <div className="mt-6 grid gap-3 md:grid-cols-2">
        {researchAndMedia.filter(([title]) => title.includes('Quantum Frontiers') || title.includes('Aaronson') || title.includes('MIT OpenCourseWare')).map(([title, description, href]) => <article key={title} className="rounded-xl border border-white/10 bg-black/20 p-5"><h3 className="text-sm font-semibold text-zinc-100">{title}</h3><p className="mt-2 text-xs leading-5 text-zinc-400">{description}</p><div className="mt-4"><ResourceLink href={href}>Open resource</ResourceLink></div></article>)}
      </div>
    </section>

    <section className="rounded-2xl border border-[#dfff3f]/15 bg-[#dfff3f]/[.03] p-6 sm:p-8">
      <div className="flex items-center gap-3"><GraduationCap className="w-5 h-5 text-[#e9ff8a]" /><div><h2 className="text-xl font-semibold text-zinc-100">Recommended QubitLab Learning Path</h2><p className="mt-1 text-sm text-zinc-400">Follow the order in the supplied resource guide.</p></div></div>
