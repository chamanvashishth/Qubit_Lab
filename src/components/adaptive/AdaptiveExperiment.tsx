...
export const AdaptiveExperiment: React.FC<Props> = ({ concept, onComplete, onAskAI }) => {
  ...
  return <section className="glass-section rounded-2xl p-6 space-y-5">
    <div className="flex flex-col md:flex-row md:items-start justify-between gap-4"><div><div className="text-[10px] uppercase tracking-[.2em] text-zinc-500 flex items-center gap-2"><FlaskConical className="w-3.5 h-3.5 text-[#dfff3f]" /> {t('Guided quantum lab')}</div><h2 className="text-xl font-semibold text-zinc-100 mt-2">{t('Predict before you run')}</h2><p className="text-sm text-zinc-400 mt-1">{concept.experiment}</p></div>{onAskAI && <button onClick={onAskAI} className="px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-xs text-zinc-300 flex items-center gap-2"><Sparkles className="w-3.5 h-3.5" /> {t('Ask the guide')}</button>}</div>
    <div className="grid lg:grid-cols-[1fr_280px] gap-5"><div><div className="rounded-xl border border-white/10 bg-black/25 p-4"><div className="text-[10px] uppercase tracking-[.16em] text-zinc-600">Circuit</div><div className="font-mono text-sm text-zinc-200 mt-2">|0{mission.qubits > 1 ? '0'.repeat(mission.qubits - 1) : ''}⟩ {mission.gates.length ? '→ ' + mission.gates.map(g => g.gate).join(' → ') : '→ inspect'}</div><div className="text-xs text-zinc-500 mt-2">{t('Prediction first. The evidence is revealed after you submit.')}</div></div>
    <div className="mt-4 space-y-2"><div className="text-[10px] uppercase tracking-[.16em] text-zinc-600">{t('What do you predict?')}</div>{mission.choices.map((choice, index) => { const active = selected === index; const state = submitted ? index === mission.correct ? 'border-emerald-500/40 bg-emerald-500/10 text-emerald-200' : active ? 'border-rose-500/40 bg-rose-500/10 text-rose-200' : 'border-white/10 bg-white/5 text-zinc-500' : active ? 'border-[#dfff3f]/50 bg-[#dfff3f]/10 text-zinc-100' : 'border-white/10 bg-white/5 text-zinc-300 hover:border-white/20'; return <button key={choice} disabled={submitted} onClick={() => setSelected(index)} className={`w-full text-left p-3 rounded-xl border text-xs transition-all ${state}`}>{choice}{submitted && index === mission.correct && <CheckCircle2 className="inline ml-2 w-4 h-4 text-emerald-400" />}{submitted && active && index !== mission.correct && <XCircle className="inline ml-2 w-4 h-4 text-rose-400" />}</button>; })}</div>
    {submitted && <div className={`mt-4 rounded-xl border p-4 text-xs leading-6 ${correct ? 'border-emerald-500/20 bg-emerald-500/5 text-emerald-100' : 'border-amber-500/20 bg-amber-500/5 text-zinc-300'}`}><b>{correct ? t('Good prediction.') : t('Useful surprise.')}</b> {mission.explanation}</div>}
    <div className="flex gap-2 mt-4">{!submitted ? <button onClick={submit} disabled={selected === null} className="template-button px-4 py-2 rounded-lg text-xs font-semibold disabled:opacity-40"><Beaker className="inline w-3.5 h-3.5 mr-1" /> {t('Run & verify')}</button> : <button onClick={reset} className="px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-xs text-zinc-300"><RotateCcw className="inline w-3.5 h-3.5 mr-1" /> {t('Try again')}</button>}</div></div>
    <div className="rounded-xl border border-white/10 bg-black/25 p-4"><div className="text-[10px] uppercase tracking-[.16em] text-zinc-600">{t('Evidence')}</div>{submitted ? <div className="space-y-3 mt-4">{evidence.map(([basis, value]) => <div key={basis}><div className="flex justify-between text-[10px] font-mono text-zinc-500"><span>|{basis}⟩</span><span>{Math.round(value * 100)}%</span></div><div className="h-2 bg-white/5 rounded-full mt-1 overflow-hidden"><div className="h-full bg-[#dfff3f]" style={{ width: `${Math.max(0, Math.min(100, value * 100))}%` }} /></div></div>)}</div> : <div className="mt-4 text-xs leading-6 text-zinc-500">{t('Choose an answer first. Once you run the mission, the simulator evidence will appear here.')}</div>}</div>
    </div>
  </section>;
};
