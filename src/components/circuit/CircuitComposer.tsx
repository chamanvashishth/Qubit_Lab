      ...
      <section className="bg-white/[.05] border border-white/10 rounded-xl p-4 backdrop-blur-sm shadow-xl">
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button onClick={() => { setGates([]); setScrubStep(null); }} className="px-3 py-1.5 rounded-lg bg-white/[.08] border border-white/10 text-xs flex items-center gap-1.5 hover:text-rose-300"><Trash2 className="w-3.5 h-3.5" />{t('Clear')}</button>
            {onAskAIExplain && <button onClick={() => onAskAIExplain(circuit, simulation.diracNotation)} className="px-3.5 py-1.5 rounded-lg bg-gradient-to-r from-purple-600 to-indigo-600 text-white text-xs flex items-center gap-1.5"><Sparkles className="w-3.5 h-3.5" />{t('Explain locally')}</button>}
          </div>
        </div>
      </section>
